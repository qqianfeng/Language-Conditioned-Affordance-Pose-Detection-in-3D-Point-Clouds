...
class _3DAPDataset(Dataset):
    ...
    def load_data(self):
        ...
        shape_ids, grasp_data = [], []
        grasp_dir = opj(self.data_dir, 'grasp_united')
        for file in os.listdir(grasp_dir):
            file_dir = opj(grasp_dir, file)
            if os.path.isfile(file_dir):
                shape_ids.append(os.path.splitext(file)[0])
                with open(file_dir, 'rb') as f:
                    grasp_data.append(pkl.load(f))
        id_grasps_dict = dict(zip(shape_ids, grasp_data))
        ...
        for id in id_grasps_dict.keys():
            for affordance in id_grasps_dict[id].keys():
                for grasp in id_grasps_dict[id][affordance]:
                    new_data_dict = {}
                    ...
                    new_data_dict['rotation'] = R.from_matrix(grasp[1][:3, :3]).as_quat()
                    new_data_dict['translation'] = grasp[1][:3, 3]
